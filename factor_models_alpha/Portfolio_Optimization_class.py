...
# -*- coding: utf-8 -*-
"""
For ECON367B Fall 2025 Single Factor Model Exercise
@author: raiyat
"""
...
class asset_return_data_class:
    def __init__(self, file_dir, index_name, T, is_portfolio,
                 ticker_list, ticker_rf, ticker_index_list,
                 start_date=datetime(2010,1,1).date(), 
                 end_date=datetime(2030,12,31).date(),
                 latest_start_date=datetime(2020,1,1).date()):
        ...
        if not is_portfolio: # asset level
            self.asset_return_df, self.asset_close_df, self.asset_ticker_df_dict = self.asset_return(index=False)
            self.asset_return_index_df, self.asset_close_index_df, self.asset_index_df_dict = self.asset_return(index=True)   
            ...
            self.asset_return_df.to_csv(file_dir+index_name+'_asset_return.csv', index=True, index_label='date')
            self.asset_return_index_df.to_csv(file_dir+index_name+'_asset_return_index.csv', index=True, index_label='date')  
            self.asset_return_df_rf.to_csv(file_dir+index_name+'_asset_return_rf.csv', index=True, index_label='date')
            self.asset_return_combined.to_csv(file_dir+index_name+'_asset_return_combined.csv', index=True, index_label='date')
            
            self.asset_close_df.to_csv(file_dir+index_name+'_asset_close.csv')
            self.asset_close_index_df.to_csv(file_dir+index_name+'_asset_close_index.csv')
           
            self.asset_des_stats = self.asset_return_combined.apply(self.asset_return_des_stats, axis=0)
            self.asset_des_stats.to_csv(file_dir+index_name+'_asset_des_statistics.csv', index=True, index_label='date')
            
            # self.asset_return_df_cov_list = self.asset_return_cov_list()
            # self.asset_return_df_cov_list.to_csv(file_dir+index_name+'asset_return_cov.csv', index=True, index_label='date')
    ...
    def asset_return(self, index): # function to get the asset return dataframe
        latest_start_date = self.latest_start_date
        ...
        for ticker in ticker_list.keys():
            ...
            ticker_df = ticker_df.loc[ticker_df.index>start_date].sort_index()
            
            # if the history of the stock is too short, ignore
            # if empty data
            try:
                # first_date = ticker_df.index[0]
                if ticker_df.index[0] >= latest_start_date: 
                    print(f'{ticker_list[ticker.ticker]} went public later than {str(latest_start_date)}')
                    continue
            except:
                print(f'{ticker_list[ticker.ticker]} has no data')
                continue
            
            ticker_close_df = ticker_df['Close'].copy(deep=True).to_frame()
            ticker_close_df.rename(columns={'Close': ticker_list[ticker.ticker]}, inplace=True)
            ticker_close.append(ticker_close_df)
            
            ticker_df[ticker_list[ticker.ticker]] = ticker_df['Close'].pct_change().dropna(how='any') 
            ticker_return.append(ticker_df[ticker_list[ticker.ticker]])
            
            ticker_df = ticker_df.dropna(how='any') 
            ticker_df.rename(columns={ticker_list[ticker.ticker]: 'Return'}, inplace=True)
            ticker_df_dict[ticker_list[ticker.ticker]] = ticker_df
        
        ticker_return = pd.concat(ticker_return, axis=1).ffill().dropna(how='any').astype(float)
        ticker_close = pd.concat(ticker_close, axis=1).ffill().dropna(how='any').astype(float)
        
        return ticker_return, ticker_close, ticker_df_dict
